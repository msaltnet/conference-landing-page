...
## 실습 소개 페이지
`/walk-through` 페이지를 추가해서 체험 내용을 소개하는 카드를 배치해줘.
- 체험 타이틀과 소개 문구는 json에 저장
- 체험 1, 2, 3, 4에 대한 타이틀과 썸네일 이미지, 소개 문구 역시 json에 저장
- 체험 내용을 소개하는 카드에는 GO 버튼이 있어서 링크로 연결됨 (링크 정보는 json에 포함)
- 영어 페이지도 지원
